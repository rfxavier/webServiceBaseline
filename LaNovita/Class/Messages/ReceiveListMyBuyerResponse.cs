...
{
    public class ReceiveListMyBuyerResponse : ServiceResponseBase
    {
        ...
        public string Apple()
        {
            ...
            if (this.BuyerList != null && this.BuyerList.Count() > 0)
            {
                for (int i = 0; i < this.BuyerList.Count(); i++)
                {
                    sb.Append("<dict>");

                    sb.Append("     <key>BuyerID</key>");
                    sb.AppendFormat("     <string>{0}</string>", this.BuyerList[i].BuyerID.ToString());
                    sb.Append("     <key>BuyerName</key>");
                    sb.AppendFormat("     <string>{0}</string>", this.BuyerList[i].BuyerName);
